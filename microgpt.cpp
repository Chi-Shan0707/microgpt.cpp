#include <iostream>
#include <vector>
#include <set> // unique charset and topo dedup
#include <map>
#include <list> // stable node addresses for pools
#include <string>
#include <cmath>
#include <random> // parameter init and token sampling
#include <functional>
#include <cstdio>
using namespace std;
struct Config { //─── Hyperparameters ──────────────────────────────────────
    int num_vocab, dim_embd, num_head, num_layer,
        dim_hidden, dim_block, num_training_steps, num_samples;
    double lr, beta1, beta2, eps;
    Config()
        : num_vocab(27), dim_embd(16), num_head(4), num_layer(4),
          dim_hidden(64), dim_block(32), num_training_steps(1000), num_samples(20),
          lr(0.01), beta1(0.9), beta2(0.999), eps(1e-8) {}
} cfg;
// ─── Autograd Engine: reverse-mode AD ─────────────────────────────────────
// autograd: build graph in forward, accumulate grads in reverse topo order
struct Value;
...
public:
    ...
    explicit Value(double d = 0.0) : data(d), grad(0.0) {}
...
class Vector { /* thin wrapper around vector<Value*> */ public: vector<Value*> data; };
...
Vector softmax(const Vector& logits) {  // numerically stable: subtract max before exp to avoid overflow
    ...
    double max_val = -1e9; for (auto v : logits.data) if (v->data > max_val) max_val = v->data; // max for stability
...
Vector relu(const Vector& input) { // ReLU activation
    ...
    for(size_t i = 0; i < input.data.size(); ++i) res.data[i] = Value::relu(input.data[i]);
    return res;
}
...
struct AttentionBlock {
    ...
    Vector forward(const Vector& x, vector<Vector>& keys, vector<Vector>& values) {
        ...
        keys.push_back(k); values.push_back(v); // causal KV cache grows token by token
        size_t seq_len = keys.size();
        Vector scores; scores.data.resize(seq_len,nullptr);
        double scale_factor = 1.0 / sqrt((double)head_dim); Value* scale_node = Value::make_new(scale_factor); // attention scaling
        for(size_t t = 0; t < seq_len; ++t) {
            Value* dot = Value::make_new(0.0);
            for(size_t i = 0; i < q.data.size(); ++i) dot = Value::add(dot, Value::mul(q.data[i], keys[t].data[i]));
...
class Tokenize{
public:
    vector<char> vocab; map<char, int> char_to_id; int BOS, EOS, UNK;
    vector<int> encode(const string& text) {
        vector<int> tokens;
        tokens.reserve(text.size() + 2);
        tokens.push_back(BOS); /* Beginning Of Sentence */
        for (char ch : text) {
            auto it = char_to_id.find(ch);
            tokens.push_back(it == char_to_id.end() ? UNK : it->second);
        }
        tokens.push_back(EOS); /* End Of Sentence */
        return tokens;
    }
...
std::function<void(Matrix&)> init_matrix = [](Matrix& m) { // Xavier-like init: normal(0, 0.2)
    static mt19937 rng(42); normal_distribution<double> dist(0.0, 0.2);
    m.data.resize(m.row, vector<Value*>(m.col));
    for (int i = 0; i < (int)m.row; ++i)
        for (int j = 0; j < (int)m.col; ++j)
            param_pool.emplace_back(dist(rng)), m.data[i][j] = &param_pool.back();
};
// inference: autoregressive decoding (predict one token at a time)
// ─── Inference ──────────────────────────────────────────────────────────────
...
string generate(GPT& model, Tokenize& tokenizer, int max_len = 16, double temp = 0.5) { // autoregressive text generation
    ...
    for (int pos = 0; pos < max_len; ++pos) {
        ...
        Vector scaled_logits = scale(1.0 / temp, logits);
...
int main(){
    ...
    tokenizer.BOS = tokenizer.vocab.size(); tokenizer.vocab.push_back('#'); /* BOS */
    tokenizer.EOS = tokenizer.vocab.size(); tokenizer.vocab.push_back('#'); /* EOS */
    tokenizer.UNK = tokenizer.vocab.size(); tokenizer.vocab.push_back('?'); /* UNK */
    ...
    cout << "\n=== Generated Samples ===" << endl; for (int i = 0; i < cfg.num_samples; ++i) cout << "  [" << i << "] " << generate(model, tokenizer, cfg.dim_block, 0.8) << endl;
    return 0;
}